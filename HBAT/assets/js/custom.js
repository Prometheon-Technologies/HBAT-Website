

setTimeout(
()=>{
    const container = document.querySelector('.user-items-list-carousel__slideshow-holder');
    let scrollAmount = 0;
    document.getElementById('slideRight').onclick = function () {
      scrollAmount += 300
      container.scrollTo({
        top: 0,
        left: scrollAmount,
        behavior: 'smooth'
      });
    };
}, 1000)

setTimeout(
  () => {
    const container = document.querySelector('.user-items-list-carousel__slideshow-holder');
    let scrollAmount = 0;
    document.getElementById('mobileSlideRight').onclick = function () {
      scrollAmount += 300
      container.scrollTo({
        top: 0,
        left: scrollAmount,
        behavior: 'smooth'
      });
    };
  }, 1000)


setTimeout(
  () => {
    const container = document.querySelector('.user-items-list-carousel__slideshow-holder');
    let scrollAmount = 0;
    document.getElementById('slideLeft').onclick = function () {
      scrollAmount -= 300
      container.scrollTo({
        top: 0,
        left: scrollAmount,
        behavior: 'smooth'
      });
    };
  }, 1000)


setTimeout(
  () => {
    const container = document.querySelector('.user-items-list-carousel__slideshow-holder');
    let scrollAmount = 0;

    document.getElementById('mobileSlideLeft').onclick = function () {
      console.log("LEFT called")

      scrollAmount -= 300
      container.scrollTo({
        top: 0,
        left: scrollAmount,
        behavior: 'smooth'
      });
    };

  }, 1000)




// setTimeout(
// ()=>{
//     var container = document.querySelector('.user-items-list-carousel__slides user-items-list-carousel__slides--initialized');
//     var scrollAmount = 0;
//     scrollAmount+=300;
//     container.style.transform = 'translate(' + scrollAmount + 'px)';

// }, 1000

// )
